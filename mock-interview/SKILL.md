...
E) 기술면접 -- 직무별 맞춤 (프로덕트(PM/PO)/기획/마케팅·그로스/데이터/개발/디자인/영업 등)
...
#### E) 기술면접
- 직무별 맞춤 질문:
  - **프로덕트(PM/PO)**: 제품 감각·우선순위·지표 해석 중심.
    - 제품 감각: "이 제품에서 지금 가장 먼저 고칠 문제 하나와 이유는?", "성공한/실패한 기능 하나를 지표로 설명해보세요"
    - 우선순위: "백로그 A/B/C 중 무엇을 먼저 하고 무엇을 버리겠나요? 기준은?", "리소스가 절반이면 스코프를 어떻게 자르나요?"
    - 지표 해석: "핵심 지표가 올랐는데 리텐션은 빠졌습니다. 어떻게 진단하나요?", "이 기능의 성공을 어떤 지표로 정의하고, 개선 폭을 어떻게 측정하나요?", "지표가 개선됐지만 우연/계절성일 가능성은 어떻게 배제하나요?"
    - 협업: "개발·디자인과 의견이 갈릴 때 무엇을 근거로 설득했나요?"
  - **기획(사업/전략)**: 사업 모델, 시장 분석, KPI 설계 + 기획 프로세스(목표설정→조사분석→아이디어→계획→실행평가) 단계별 역할
  - **마케팅/그로스**: 퍼널·실험 설계, ROI 분석, 고객 세그먼트 + 성과 수치와 본인 기여도 분리 확인
  - **개발**: 시스템 설계, 알고리즘, 기술 선택 이유, 트레이드오프
  - **데이터 분석**: 지표 정의, 실험 설계·해석(인과 vs 상관), SQL/대시보드로 의사결정을 바꾼 경험
